...
export const Route = createFileRoute('/cidade/$cidade')({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) return { meta: [{ title: 'Cidade — Studio' }] }
    const { cidade } = loaderData
    const titulo = `Acompanhantes em ${cidade.nome} (${cidade.estado}) — Studio`
    return {
      meta: [
        { title: titulo },
      ...
      scripts: [
        {
          ...
          children: JSON.stringify({
            ...
            '@type': 'BreadcrumbList',
            itemListElement: [
              { '@type': 'ListItem', position: 1, name: 'Studio', item: '/' },
  ...
  return (
    <div className="mx-auto max-w-6xl px-5 py-10">
      <nav className="text-xs text-muted">
        <Link to="/" className="hover:text-ink">Studio</Link> /{' '}
