...
export const Route = createFileRoute('/acompanhantes/$slug')({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) return { meta: [{ title: 'Perfil — Studio' }] }
    const cidade = loaderData.cidade ? cidadePorSlug(loaderData.cidade) : undefined
    const titulo = `${loaderData.nome_exibicao}${cidade ? ` — Acompanhante em ${cidade.nome}` : ''} | Studio`
    const desc =
      loaderData.bio?.slice(0, 150) ??
      `Perfil verificado de ${loaderData.nome_exibicao} no Studio. Fotos reais e contato direto pelo WhatsApp.`
